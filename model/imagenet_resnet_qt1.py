...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes_1, planes_2, planes_3, ka_1, ka_2, ka_3,  stride=1, downsample=None):
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes, filters_left, ka, kw):
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], slim_channel=filters_left[0:3 * 3 + 2], ka = ka[1:3 * 3 + 2])
        self.layer2 = self._make_layer(block, 128, layers[1], slim_channel=filters_left[3 * 3 + 1:3 * 7 + 3], ka = ka[3 * 3 + 2:3 * 7 + 3], stride=2)
        self.layer3 = self._make_layer(block, 256, layers[2], slim_channel=filters_left[3 * 7 + 2:3 * 13 + 4], ka = ka[3 * 7 + 3:3 * 13 + 4], stride=2)
        self.layer4 = self._make_layer(block, 512, layers[3], slim_channel=filters_left[3 * 13 + 3:3 * 16 + 5], ka = ka[3 * 13 + 4:3 * 16 + 5], stride=2)
    ...
    def _make_layer(self, block, planes, blocks, slim_channel, ka, stride=1):
        # downsample = None
        # if stride != 1 or self.inplanes != planes * block.expansion:
        #     downsample = nn.Sequential(
        #         nn.Conv2d(slim_channel[0], planes * block.expansion,
        ...
        layers = []
        layers.append(block(slim_channel[0], slim_channel[1], slim_channel[2], slim_channel[3], ka[0], ka[1], ka[2], stride, downsample))
        ...
        for i in range(1, blocks):
            ...
            ka_2 = ka[3*i+2]
            ka_3 = ka[3*i+3]
            layers.append(block(inplanes, planes_1, planes_2, planes_3, ka_1, ka_2, ka_3, stride = 1, downsample = None))
...
def qresnet50_A(filters_left, bit, pretrained=False, **kwargs):
