...
class ResBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes_1, planes_2, stride, ka_1, ka_2):
...
class QResNet_A(nn.Module):
    def __init__(self, block, num_layers, covcfg, num_classes, filters_left, ka, kw):
        ...
        self.layer1 = self._make_layer(block, 16, blocks=n, stride=1, slim_channel=filters_left[1:2 * n + 1], ka = ka[1:2 * n + 1])
        self.layer2 = self._make_layer(block, 32, blocks=n, stride=2, slim_channel=filters_left[2 * n + 1:4 * n + 1], ka = ka[2 * n + 1:4 * n + 1])
        self.layer3 = self._make_layer(block, 64, blocks=n, stride=2, slim_channel=filters_left[4 * n + 1:6 * n + 1], ka = ka[4 * n + 1:6 * n + 1])
    ...
    def _make_layer(self, block, planes, blocks, stride, slim_channel, ka):
        layers = []
        layers.append(block(self.inplanes, slim_channel[0], slim_channel[1], stride, ka[0], ka[1]))
        ...
        for i in range(1, blocks):
            ...
            layers.append(block(inplanes, planes_1, planes_2, 1, ka_1, ka_2))
...
def qresnet_56_A(num_classes, filters_left, bit):
    cov_cfg = [(3 * i + 2) for i in range(9 * 3 * 2 + 1)]
    return QResNet_A(ResBasicBlock, 56, cov_cfg, num_classes=num_classes, filters_left=filters_left, ka=bit, kw=bit)


def qresnet_110_A(num_classes, filters_left, bit):
    cov_cfg = [(3 * i + 2) for i in range(18 * 3 * 2 + 1)]
    return QResNet_A(ResBasicBlock, 110, cov_cfg, num_classes=num_classes, filters_left=filters_left, ka=bit, kw=bit)
