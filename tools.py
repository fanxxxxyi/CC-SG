...
def get_total_anchor_size(multi_level=False, name='VOC', version=None):
    if name == 'VOC':
        if multi_level:
            ...
            elif version == 'tiny_yolo_v3' or version == 'mobilenetv1_v3head':
                all_anchor_size = TINY_MULTI_ANCHOR_SIZE
            else:
                print('Unknown Version !!!')
                exit(0)
        else:
            if version == 'yolo_v2' or version == 'slim_yolo_v2':
                all_anchor_size = ANCHOR_SIZE
            elif version == 'mobilenetv1_v2head':
                all_anchor_size = ANCHOR_SIZE_MBV1
            else:
                print('Unknown Version !!!')
                exit(0)
